...
def main():
    # simulate the process of making a BN:
    # 1. reconnect the poor nodes
    # 2. tame the rich nodes
    graph0 = nx.read_gpickle("Decision-1.1.0_graph_0")
    make_BN.tame_rich(graph0)
    dot_sourcecode = make_grid_layout(graph0)
    with open("graph0.dot", "w+") as f:
        f.write(dot_sourcecode)
    print("done!")
