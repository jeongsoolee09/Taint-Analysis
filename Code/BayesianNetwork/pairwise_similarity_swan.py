...
def print_metadata(df):
    ...
    lengths_series = df.applymap(len)
    no_sims = lengths_series[lengths_series["similar_indices"] == 0]
    with_similars_number = df.shape[0]
    no_similars_number = lengths_series.shape[0]
    maximum_similar_number = lengths_series.max().get("similar_indices")
    minimum_similar_number = lengths_series.min().get("similar_indices")
    print("There are {} nodes with at least one similar node".format(with_similars_number))
    print("There are {} nodes without any similar nodes".format(no_similars_number))
    print("There were maximum {} similar nodes for a node".format(maximum_similar_number))
    print("There were minimum {} similar nodes for a node".format(minimum_similar_number))
...
def main():
    df = score_all_rows()
    print_metadata(df)
    df.to_csv("pairwise_sims.csv", mode="w+")
