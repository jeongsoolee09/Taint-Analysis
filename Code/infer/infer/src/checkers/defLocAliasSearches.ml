...
let search_target_tuple_by_pvar (pvar:Var.t) (methname:Procname.t) (astate_set:S.t) : T.t =
  ...
  let rec inner pvar (methname:Procname.t) elements =
    ...
    | ((procname, _, _, aliasset) as target)::t ->
        if Procname.equal procname methname && A.mem (pvar, []) aliasset then target else inner pvar methname t in
...
let search_target_tuples_by_pvar (pvar:Var.t) (methname:Procname.t) (tupleset:S.t) : T.t list =
  ...
  let rec inner pvar (methname:Procname.t) elements =
    ...
    | ((procname, _, _, aliasset) as target)::t ->
        ...
        then target::inner pvar methname t
        else inner pvar methname t in
  inner pvar methname elements
...
let search_target_tuple_by_id (id:Ident.t) (methname:Procname.t) (astate_set:S.t) : T.t =
  ...
  let rec inner id (methname:Procname.t) elements =
    ...
    | [] -> (* L.die InternalError "search_target_tuple_by_id failed, id: %a, methname: %a, tupleset: %a@." Ident.pp id Procname.pp methname S.pp astate_set *)
      raise IDontKnow
    | ((procname, _, _, aliasset) as target)::t ->
        if Procname.equal procname methname && A.mem (Var.of_id id, []) aliasset then target else inner id methname t in
...
let search_target_tuple_by_pvar_ap (ap:MyAccessPath.t) (methname:Procname.t) (astate_set:S.t) : T.t =
  ...
  let rec inner pvar (methname:Procname.t) elements =
    ...
    | ((procname, _, _, aliasset) as target)::t ->
        if Procname.equal procname methname && A.mem ap aliasset then target else inner pvar methname t in
...
let search_target_tuples_by_id (id:Ident.t) (methname:Procname.t) (astate_set:S.t) : T.t list =
  ...
  let rec inner id (methname:Procname.t) elements acc =
    ...
    | ((procname, _, _, aliasset) as target)::t ->
        ...
        then inner id methname t (target::acc)
        else inner id methname t acc in
  inner id methname elements []
...
let find_tuples_with_ret (tupleset:S.t) (methname:Procname.t) : T.t list =
  let elements = S.elements tupleset in
  let rec inner (tuplelist:T.t list) (methname:Procname.t) (acc:T.t list) =
    match tuplelist with
    | [] -> acc
    | (procname, _, _, aliasset) as target :: t ->
        if Procname.equal procname methname && A.exists is_return_ap aliasset
        then inner t methname (target::acc)
        else inner t methname acc in
  inner elements methname []
...
let search_target_tuples_by_vardef (pv:Var.t) (methname:Procname.t) (tupleset:S.t) : T.t list =
  ...
  let rec inner pv (methname:Procname.t) elements acc =
    ...
    | ((procname, (vardef, _), _, _) as target) :: t ->
        ...
        then inner pv methname t (target::acc)
        else inner pv methname t acc in
...
let search_target_tuples_by_vardef_ap (pv_ap:MyAccessPath.t) (methname:Procname.t) (tupleset:S.t) : T.t list =
  ...
  let rec inner pv_ap methname elements acc =
    ...
    | ((procname, var_ap, _, _) as target) :: t ->
        ...
        then inner pv_ap methname t (target::acc)
        else inner pv_ap methname t acc in
...
let find_least_linenumber (statelist:T.t list) : T.t =
  let rec inner (statelist:T.t list) (current_least:T.t) : T.t =
    ...
    | targetState::t ->
        ...
        then inner t targetState
        else inner t current_least in
...
let find_another_pvar_vardef (varset:A.t) : A.elt =
  ...
  let rec inner (varlist:A.elt list) =
    ...
    | (var, _) as target::t -> if is_program_var var then target else inner t in
  inner varlist
...
let find_pvar_ap_in (aliasset:A.t) : A.elt =
  ...
  | [] -> acc
  | (var, _) as target::t ->
      if is_program_var var then target::acc else inner t acc in
...
let find_ap_with_field (aliasset:A.t) : A.elt =
  let elements = A.elements aliasset in
    ...
    | (_, aplst) as target :: t -> if List.length aplst > 0 then target else inner t in
  inner elements
