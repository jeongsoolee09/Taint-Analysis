...
let to_json_repr (hashtbl : (Procname.t, Annot.Method.t) Hashtbl.t) : json =
  let to_key_attr meth annot =
    if not @@ Annot.Method.is_empty annot then
      (Procname.to_string meth, `String (F.asprintf "%a" (Annot.Method.pp "") annot))
    else (Procname.to_string meth, `String "no annotation")
  in
  let key_attr_list = Hashtbl.fold (fun k v acc -> to_key_attr k v :: acc) hashtbl [] in
  `Assoc key_attr_list
