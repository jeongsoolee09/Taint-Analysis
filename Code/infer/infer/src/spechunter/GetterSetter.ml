...
type methods = Getter of Procname.t | Setter of Procname.t | Nothing

(** Procname.t에서 그 써머리로 가는 테이블. *)
let method_summary_table = Hashtbl.create 777

let get_summary (key : Procname.t) : S.t =
  try Hashtbl.find method_summary_table key with _ -> S.empty


(** 주어진 methname을 가진 메소드가 단순 getter인지를 판단한다. 검출 방법:
    (this, [fieldName]) 와 return 각각의 aliasset이 동일한 logicalvar를 갖고 있는지를
    확인. *)
let detect_getter (methname : Procname.t) : methods option =
  ...
  | _ -> (
      ...
      let this_fieldname_tuples =
        S.fold
          (fun tup acc ->
            let aliasset = fourth_of tup in
            let is_this_fieldname =
              A.fold
                (fun aliastup acc ->
                  acc || ((Var.is_this @@ fst aliastup) && (not @@ List.is_empty @@ snd aliastup)))
                aliasset false
            in
            (tup, is_this_fieldname) :: acc)
          summary []
      in
      (* true flag와 결부된 튜플만 필터링 *)
      let this_fieldname_tuples' =
        List.filter ~f:(fun tup -> Bool.equal true @@ snd tup) this_fieldname_tuples
      in
      let this_fieldname_tuple =
        match this_fieldname_tuples' with [tuple] -> fst tuple | _ -> bottuple
      in
      (* 나중에 false를 리턴하게 할 것 *)
      ...
      let return_tuples =
        S.fold
          (fun tup acc ->
            let aliasset = fourth_of tup in
            let is_return =
              A.fold (fun aliastup acc -> acc || (Var.is_return @@ fst aliastup)) aliasset false
            in
            (tup, is_return) :: acc)
          summary []
      in
      ...
      let return_tuple = match return_tuples' with [tuple] -> fst tuple | _ -> bottuple in
      (* 나중에 false를 리턴하게 할 것 *)

      (* 해당 안 되면 일찍 빠지시고요 *)
      if T.equal this_fieldname_tuple bottuple || T.equal return_tuple bottuple then Some Nothing
      else
        ...
        | [] ->
            Some Nothing
        | _ ->
            Some (Getter methname) )


(** 주어진 methname을 가진 메소드가 단순 setter인지를 판단한다. 검출 방법:
    (this, [fieldName])이 vardef로 들어 있는 튜플이 파라미터인 fieldName을 가지고
    있고, 그 line number는 fieldName이 정의된 line number보다 크거나 같다. *)
let detect_setter (methname : Procname.t) : methods option =
  ...
  | _ ->
      ...
      let sanity =
        List.fold_left
          ~f:(fun acc tup ->
            if (is_logical_var @@ fst tup) || (Var.is_this @@ fst tup) then acc else tup :: acc)
          ~init:[] alias_tuples
      in
      let sanity_check = Int.equal 1 @@ List.length sanity in
      if not sanity_check then Some Nothing
      else
        (* 중간 sanity check 들어가실게요 *)

        (* Phase 2: *하나만* 있는 그 튜플 (파라미터)의 line number가 Phase 1에서 발견한 튜플의 line number 이전인지 확인한다. *)
        let ap = List.nth_exn sanity 0 in
        let target_tuple =
          try search_target_tuple_by_vardef_ap ap methname summary with _ -> bottuple
        in
        if T.equal target_tuple bottuple then Some Nothing
        else
...
let catMaybes (option_list : 'a option list) : 'a list =
  List.fold
    ~f:(fun acc elem -> match elem with Some a -> a :: acc | None -> acc)
    ~init:[] option_list
...
let collect_getter (meths : Procname.t list) : methods list =
  ...
  |> List.filter ~f:(fun meth -> match meth with Getter _ -> true | _ -> false)


(** Procname들 중에서 Setter만을 모은다. *)
let collect_setter (meths : Procname.t list) : methods list =
  catMaybes @@ List.map ~f:detect_setter meths
  |> List.filter ~f:(fun meth -> match meth with Setter _ -> true | _ -> false)


(** Getter 또는 Setter로 label된 method 한 개를 json 형식으로 찍어낸다. key: method
    name, attr: "getter" 혹은 "setter". *)
let to_json_repr (labelled_methods : methods list) : json =
  let to_key_attr labelled_method =
    ...
    | Nothing ->
        L.die InternalError "trying to write a non-getter/setter method\n"
  in
  let key_attr_list =
    List.fold
      ~f:(fun acc labelled_method -> to_key_attr labelled_method :: acc)
      ~init:[] labelled_methods
  in
  `Assoc key_attr_list
