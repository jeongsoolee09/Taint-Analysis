...
let alias_with_callv (statetup : S.elt) : bool =
  let aliasset = fourth_of statetup in
  A.fold (fun ap acc -> acc || is_callv_ap ap) aliasset false
...
let rec compute_chain_inner (current_methname : Procname.t) (current_astate_set : S.t)
  ...
  let var_aps =
    collect_program_var_aps_from current_aliasset_cleanedup ~self:current_vardef ~just_before
  in
  ...
  | [] ->
      ...
      if exists ~f:(fun (var, _) -> Var.is_return var) var_aps then
      ...
      else if exists ~f:(fun ap -> is_param_ap ap) var_aps then
        (* if one or more param variables exist for a single callee,
           then we don't have summary for that callee *)
        let param_ap_in_question = find_witness_exn var_aps ~pred:is_param_ap in
        ...
        fold
          ~f:(fun acc (callee, callee_astate) ->
            ...
            if have_been_before callee current_chain then acc
            else
              ...
              try
                ...
                in
                compute_chain_inner current_methname current_astate_set alias_with_returnv
                  chain_updated retry
              with IBase.Die.InferInternalError _ -> acc)
