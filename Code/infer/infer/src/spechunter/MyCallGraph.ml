(** A Very Naive Call Graph Module: * 각 파일의 각 Procdesc.t의 각 노드의 각 instr를 뽑아서 해당 메소드의 콜리를 계산해 낸다. **)
...
let load_callgraph_from_disk_to hashtbl ~(exclude_test : bool) =
  let callees_and_callers =
    ...
    |> List.concat
    |> List.map ~f:(fun pname -> (pname, Procdesc.load pname))
    |> List.filter ~f:(fun (_, opt) -> Option.is_some opt)
    |> catMaybes_tuplist
    |> List.map ~f:(fun (p, pdesc) -> (p, Procdesc.get_static_callees pdesc))
  in
  List.iter callees_and_callers ~f:(fun (k, values) ->
      List.iter
        ~f:(fun v ->
          match
            (exclude_test, TestCodeDetector.is_test_method k || TestCodeDetector.is_test_method v)
          with
          | true, true ->
              ()
          | true, false ->
...
let load_callgraph_from_disk_to_map map =
  ...
  in
  List.fold
    ~f:(fun acc (caller, callees) ->
      List.fold ~f:(fun acc_ callee -> Map.add caller callee acc_) ~init:acc callees )
    ~init:map callees_and_callers
