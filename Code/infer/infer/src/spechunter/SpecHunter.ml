...
let run_only_when_not_file_exists (f : unit -> unit) (filename : string) : unit =
  ...
  | `No | `Unknown ->
      f ()
  | `Yes ->
      L.progress "Skipped calculating %s because it's already there.@." filename
...
let main () =
  ...
  run_only_when_not_file_exists ReturnStatementLines.main "return_stmt_locations.json"
